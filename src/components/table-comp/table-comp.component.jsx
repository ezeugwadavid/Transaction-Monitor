   ...
   return (
	<TableContainer>
        ...
        <div className="table-nav">
            ...
            <div className="t2">20 <span className='arr-down'><img src={ArrowDownBlue} alt="" /> </span></div>
            <div className="t3">out of 500 payments</div>

            <div className="nav-center">
            <img className='search-svg' src={Search} alt="" />
            <input className='table-search' placeholder='Search payments' type="search" />
            </div>
            ...
            <div className="nav-end">
             Show <span className='show-arr-down' onClick={() => setShowSort(!showSort)}><img src={ArrowDownBlue} alt="" /></span>
            </div>
      ...
      <tbody>
        { dummyData.map(data => {
          ...
          <td><div className="price">{data.price}</div></td>
          <td><div className="trans">{data.transactionnumber}</div></td>
          <td><div className="time">{data.time}</div></td>
          <td>
            <div className="state">
            <div className={data.type === 'Pending' ? 'circle-dot-yellow' : data.type === 'Reconcilled' ? 'circle-dot-green' : data.type === 'Un-reconcilled' ? 'circle-dot-grey' : 'circle-dot-yellow'}></div>
            <div className={data.type === 'Pending' ? 'state-txt-yellow' : data.type === 'Reconcilled' ? 'state-txt-green' : data.type === 'Un-reconcilled' ? 'state-txt-grey' : 'state-txt-yellow'}>{data.type}</div>
            </div>
            </td>

            <td><img className='arr-down-lg' src={ArrowDownLg} alt="" /></td>
        </tr>
        })
